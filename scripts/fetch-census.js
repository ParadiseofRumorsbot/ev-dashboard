#!/usr/bin/env node
/**
 * US Census Bureau Import Data Fetcher v4
 * 2차전지 밸류체인 + 차량 + 알루미늄 + 솔라 + 로보틱스 수입 데이터 수집
 *
 * Usage: CENSUS_API_KEY=xxxxx node scripts/fetch-census.js
 *
 * v4 변경사항:
 *  - 알루미늄·솔라·로보틱스 HS코드 추가 (총 30개)
 *  - companies 메타데이터 추가 (전종목)
 *  - GEN_QY1_MO 파싱 보강 (중량 단가 계산용)
 *  - 캄보디아·필리핀 감시국 추가 (솔라 우회)
 */
const fs = require('fs');
...
const HS_CODES = {
  ...
  '380110':     { label:'인조흑연 (음극재)',     labelEn:'Artificial Graphite',       level:'HS6',  tab:'material',
                  companies:['포스코퓨처엠','대주전자재료','Shanghai Putailai','BTR','Showa Denko'] },
  '250410':     { label:'천연흑연 분말',         labelEn:'Natural Graphite Powder',   level:'HS6',  tab:'material',
                  companies:['포스코퓨처엠','Syrah Resources','Northern Graphite','Nouveau Monde'] },
  '3824999397': { label:'전해액 (화학조제품)',    labelEn:'Electrolyte (Chem Prep)',   level:'HS10', tab:'material',
                  companies:['솔브레인','천보','동화일렉트로라이트','Enchem'] },
  '741012':     { label:'전해동박',              labelEn:'Electrodeposited Cu Foil',  level:'HS6',  tab:'material',
                  companies:['롯데에너지머티리얼즈','솔루스첨단소재','SKC','일진머티리얼즈','Mitsui Mining'] },
  '760711':     { label:'알루미늄박 (양극박)',    labelEn:'Rolled Al Foil',            level:'HS6',  tab:'material',
                  companies:['삼아알미늄','동일알미늄','Showa Denko','UACJ Foil'] },

  // ── 원료 ──
  '283691':     { label:'탄산리튬',              labelEn:'Lithium Carbonate',         level:'HS6',  tab:'raw',
                  companies:['앨버말','SQM','Pilbara','Arcadium Lithium','강봉리튬'] },
  '282520':     { label:'수산화리튬',            labelEn:'Lithium Hydroxide',         level:'HS6',  tab:'raw',
                  companies:['앨버말','SQM','강봉리튬','Pilbara','포스코퓨처엠'] },
  '283324':     { label:'황산니켈',              labelEn:'Nickel Sulphate',           level:'HS6',  tab:'raw',
                  companies:['에코프로비엠','GEM','Sumitomo Metal Mining','Glencore'] },
  '282200':     { label:'산화코발트',            labelEn:'Cobalt Oxide',              level:'HS6',  tab:'raw',
                  companies:['성일하이텍','Umicore','Glencore','CMOC'] },

  // ── 차량 ──
  '870380':     { label:'BEV (순수전기차)',      labelEn:'Battery Electric Vehicle',  level:'HS6',  tab:'vehicle',
                  companies:['현대차','기아','BMW','Mercedes','Volvo','Tesla'] },
  '870340':     { label:'PHEV (가솔린+전기)',    labelEn:'PHEV Gasoline Plug-in',     level:'HS6',  tab:'vehicle',
                  companies:['현대차','기아','BMW','Mercedes','Volvo','Toyota'] },
  '870350':     { label:'PHEV (디젤+전기)',      labelEn:'PHEV Diesel Plug-in',       level:'HS6',  tab:'vehicle',
                  companies:['BMW','Mercedes','Volvo'] },
  '870360':     { label:'HEV (가솔린 하이브리드)',labelEn:'HEV Gasoline Non-Plugin',   level:'HS6',  tab:'vehicle',
                  companies:['Toyota','현대차','기아','Honda','Ford'] },
  '870321':     { label:'ICE 가솔린 ≤1L',       labelEn:'ICE Gasoline ≤1000cc',      level:'HS6',  tab:'vehicle', companies:[] },
  '870322':     { label:'ICE 가솔린 1~1.5L',    labelEn:'ICE Gasoline 1000-1500cc',  level:'HS6',  tab:'vehicle', companies:[] },
  '870323':     { label:'ICE 가솔린 1.5~3L',    labelEn:'ICE Gasoline 1500-3000cc',  level:'HS6',  tab:'vehicle', companies:[] },
  '870324':     { label:'ICE 가솔린 >3L',       labelEn:'ICE Gasoline >3000cc',      level:'HS6',  tab:'vehicle', companies:[] },

  // ── 알루미늄 (신규) ──
  '760612':     { label:'합금 판·시트·스트립',    labelEn:'Al Alloy Plate/Sheet/Strip', level:'HS6',  tab:'aluminum',
                  companies:['조일알미늄','삼아알미늄','남선알미늄','알멕','대호에이엘','Alcoa','Constellium','Norsk Hydro','Hindalco','Novelis','UACJ','Kobe Steel','Kaiser Aluminum'] },
  '760200':     { label:'폐기물·스크랩',          labelEn:'Al Waste & Scrap',           level:'HS6',  tab:'aluminum',
                  companies:['조일알미늄','삼아알미늄','남선알미늄','알멕','피제이메탈','Alcoa','Constellium','Norsk Hydro','Hindalco','Kaiser Aluminum','UACJ'] },

  // ── 솔라 (신규) ──
  '854143':     { label:'태양광 모듈',            labelEn:'PV Modules (Assembled)',     level:'HS6',  tab:'solar',
                  companies:['한화솔루션','LONGi','JA Solar','Trina Solar','Canadian Solar','First Solar','Jinko Solar'] },
  '854142':     { label:'태양전지 셀',            labelEn:'PV Cells (Unassembled)',     level:'HS6',  tab:'solar',
                  companies:['대주전자재료','한화솔루션','LONGi','Tongwei','Aiko Solar','Meyer Burger'] },
  '280461':     { label:'폴리실리콘',             labelEn:'Polysilicon (≥99.99%)',      level:'HS6',  tab:'solar',
                  companies:['OCI홀딩스','한화솔루션','Wacker','Hemlock','Tongwei','대전실리콘'] },

  // ── 로보틱스·산업재 (신규) ──
  '848340':     { label:'감속기·기어박스',         labelEn:'Gears/Gearboxes/Speed Changers', level:'HS6', tab:'robotics',
                  companies:['에스피지','삼익THK','하모닉드라이브','나부테스코','Nidec','Sumitomo Drive','SEW-EURODRIVE'] },
  '850152':     { label:'서보모터 (AC 0.75~75kW)',labelEn:'AC Motors 750W-75kW',        level:'HS6',  tab:'robotics',
                  companies:['LS ELECTRIC','현대일렉트릭','Yaskawa','Fanuc','Siemens','ABB','Mitsubishi Electric'] },
  '850511':     { label:'희토류 영구자석',         labelEn:'Permanent Magnets (Metal)',   level:'HS6',  tab:'robotics',
                  companies:['성림첨단산업','Star Group Industrial','TDK','Shin-Etsu','Hitachi Metals','Neo Performance'] },
};

// ══════════════════════════════════════════════
//  주요 감시 국가 (캄보디아·필리핀 추가)
// ══════════════════════════════════════════════
const WATCH = {
  ...
  'CAMBODIA':    { code:'KH', label:'캄보디아',  group:'circumvention' },
  'PHILIPPINES': { code:'PH', label:'필리핀',    group:'circumvention' },
};
...
async function fetchMonth(hs, info, time) {
  const url = `${BASE}?get=I_COMMODITY,CTY_CODE,CTY_NAME,GEN_VAL_MO,GEN_QY1_MO,GEN_QY1_MO_FLAG`
    + `&I_COMMODITY=${hs}&time=${time}&COMM_LVL=${info.level}&SUMMARY_LVL=DET&key=${API_KEY}`;
  try {
    const d = await fetchRetry(url);
    if (!d || d.length < 2) return null;
    const h = d[0];
    const ci = h.indexOf('CTY_NAME');
    const vi = h.indexOf('GEN_VAL_MO');
    const qi = h.indexOf('GEN_QY1_MO');
    const fi = h.indexOf('GEN_QY1_MO_FLAG');
    const res = { total: 0, totalQty: 0, qtyUnit: '', countries: {} };
    for (let i = 1; i < d.length; i++) {
      const name = d[i][ci];
      const val = parseInt(d[i][vi]) || 0;
      const qty = parseInt(d[i][qi]) || 0;
      const flag = fi >= 0 ? (d[i][fi] || '') : '';
      if (name === 'TOTAL FOR ALL COUNTRIES') {
        res.total = val;
        res.totalQty = qty;
        if (flag) res.qtyUnit = flag;
        continue;
      }
      const w = WATCH[name];
...
async function main() {
  console.log('══════════════════════════════════════════════════════');
  console.log('  Census Import Fetcher v4');
  console.log('  셀·소재·원료·차량·알루미늄·솔라·로보틱스 — 30개 코드');
  console.log('══════════════════════════════════════════════════════');
  ...
  const out = {
    meta: {
      ...
      tabs: {
        ...
        aluminum: { label:'알루미늄',       codes: entries.filter(([,v])=>v.tab==='aluminum').map(([k])=>k) },
        solar:    { label:'솔라',           codes: entries.filter(([,v])=>v.tab==='solar').map(([k])=>k) },
        robotics: { label:'로보틱스·산업재', codes: entries.filter(([,v])=>v.tab==='robotics').map(([k])=>k) },
      },
    },
    monthly: {},
  };
  ...
  for (const [hs, info] of entries) {
    ...
    for (const t of months) {
      ...
      calls++;
      if (r && r.total > 0) { out.monthly[hs][t] = r; process.stdout.write(` $${(r.total/1e6).toFixed(1)}M${r.totalQty>0?' Q:'+r.totalQty:''}\n`); }
